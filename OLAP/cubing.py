...
DB_PATH: pathlib.Path = WAREHOUSE_DIR / "data/dw/my_datawarehouse.db"
...
def ingest_sales_data_from_dw() -> pd.DataFrame:
    ...
    try:
        ...
        sales_df = pd.read_sql_query(
            "SELECT sale_date, product_id, sales_amount, customer_id FROM sale", conn
        )
        conn.close()
...
def ingest_customers_data_from_dw() -> pd.DataFrame:
    """Ingest sales data from SQLite data warehouse."""
    try:
        conn = sqlite3.connect(DB_PATH)
        customers_df = pd.read_sql_query(
            "SELECT customer_id, age, region, gender, name FROM customers", conn
        )
        conn.close()
...
def create_olap_cube(sales_df: pd.DataFrame, dimensions: list, metrics: dict) -> pd.DataFrame:
    ...
    try:
        ...
        cube["customer_ids"] = grouped["product_id"].apply(list).reset_index(drop=True)

        # Generate explicit column names
        explicit_columns = generate_column_names(dimensions, metrics)
        explicit_columns.append("customer_ids")  # Include the traceability column
...
def main():
    ...
    dimensions = ["age", "gender", "region", "customer_id"]
